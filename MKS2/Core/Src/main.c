...
static void blink(){
  ...
  if(Tick > lastBlinkTick + BLINK_TIME){
    lastBlinkTick = Tick;
...
int main(void)
{
  ...
  while (1)
  {
    ...
    // LED1 blinking function
    blink();

    // Button hanler function with debouncing
    static uint32_t lastButtonTick2;
    if(Tick > lastButtonTick2 + BUTTON_PERIOD){
      lastButtonTick2 = Tick;
      button();
    }
